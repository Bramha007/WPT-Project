# File: src/dataio/voc_parser.py (FINAL, ROBUST VERSION)
...
def parse_voc(xml_path: str) -> dict:
    ...
    Returns: {"width": w, "height": h, "boxes": [], "labels_wxh_str": []}
    """
    ...
    for obj in root.findall("object"):
        ...
        if xmax > xmin and ymax > ymin:
            ...
            # --- ROBUST WxH Extraction from <attributes> ---
            try:
                attrs = obj.find("attributes")
                # Ensure we find the text from the attributes tags
                obj_w = attrs.find("width").text
                obj_h = attrs.find("height").text
                
                # Create a consistent WxH label string (e.g., '32x64')
                labels_wxh_str.append(f"{obj_w}x{obj_h}")
            except AttributeError:
                # Handle cases where an object is missing the <attributes> tag
                print(f"Warning: Object in {xml_path} missing WxH attributes. Skipping box.")
                # We need to remove the box if we skip the label to keep lists aligned
                boxes.pop() 
